...
export default function CreateGroupForm() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (res.ok) { router.push(`/grupos/${d.slug}`); router.refresh(); }
  ...
  return (
    <form onSubmit={submit} className="space-y-3 rounded-2xl border border-white/10 bg-navy-2/50 p-5 max-sm:rounded-none max-sm:border-x-0">
      <h2 className="text-sm font-semibold text-white">Nuevo grupo</h2>
      <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nombre del grupo" maxLength={50} className={input} />
      ...
      <label className="flex items-center gap-3 text-sm text-white/80">
        <input type="checkbox" checked={isPrivate} onChange={(e) => setIsPrivate(e.target.checked)} className="h-4 w-4 accent-purple" />
        Privado (las publicaciones solo las ven miembros)
      </label>
